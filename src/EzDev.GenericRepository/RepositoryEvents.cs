...
public class RepositoryEvents<TEntity> {
    ...
    /// </summary>
    public Func<TEntity, Task> OnBeforeSaving { get; init; } = _ => Task.CompletedTask;
    /// <summary>
    /// Triggered when the add and update save operation succeeded. 
    /// </summary>
    public Func<TEntity, Task> OnSaved { get; init; } = _ => Task.CompletedTask;
    /// <summary>
    /// Triggered when the add and update save operation failed.
    /// </summary>
    public Func<TEntity, DbUpdateException, Task> OnSavingFailed { get; init; } = (_,_) => Task.CompletedTask;
}